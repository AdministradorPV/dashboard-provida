...
const MasterView = ({ data }: MasterViewProps) => {
  ...
  // Prepare chart data
  const revenueByUnit = units.map(u => ({
    name: u.name,
    value: u.revenue
  })).sort((a, b) => b.value - a.value);

  const ebitdaByUnit = units.map(u => ({
    name: u.name,
    value: u.ebitda
  })).sort((a, b) => b.value - a.value);
  ...
  const sortedExpenses = [...consolidated.expenses].sort((a, b) => b.value - a.value);

  const COLORS = ['#3b82f6', '#8b5cf6', '#ec4899', '#f43f5e', '#f97316', '#eab308', '#22c55e', '#14b8a6', '#06b6d4', '#6366f1'];
  ...
  return (
    <div className="view-container">
      ...
      <div className="grid-2">
        ...
        <div className="glass-panel chart-container" style={{height: '800px'}}>
          <h3 className="chart-title">Comparativo de EBITDA</h3>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={ebitdaByUnit} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }} barSize={20}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.1)" horizontal={false} />
              <XAxis type="number" hide />
              <YAxis type="category" dataKey="name" width={180} tick={{fill: '#94a3b8', fontSize: 11}} interval={0} />
              <Tooltip 
        ...
        <table className="data-table">
          ...
          <tbody>
            <tr className="row-highlight">
              <td>Receita Bruta</td>
              <td>{formatCurrency(consolidated.revenue)}</td>
              <td>100%</td>
            </tr>
            {sortedExpenses.map((exp, i) => (
              <tr key={i}>
                ...
                <td>{exp.percentOfRevenue.toFixed(1)}%</td>
              </tr>
            ))}
            <tr className="row-highlight positive">
              <td>EBITDA</td>
              <td>{formatCurrency(consolidated.ebitda)}</td>
              <td>{calculateMargin(consolidated.ebitda, consolidated.revenue)}</td>
            </tr>
            <tr className="row-highlight">
               <td>Resultado Líquido</td>
               <td>{formatCurrency(consolidated.netProfit)}</td>
               <td>{((consolidated.netProfit / consolidated.revenue) * 100).toFixed(1)}%</td>
            </tr>
          </tbody>
        </table>
      </div>
