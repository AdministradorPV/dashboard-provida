...
const AnalyticsView = ({ data }: AnalyticsViewProps) => {
  ...
  const formatValue = (val: number) => {
    if (metric === "percent") return val.toFixed(2) + "%";
    ...
    <div className="view-container">
      ...
      <div className="grid-2">
        ...
        >
          <div className="flex-header">
            <h3 className="chart-title">
              <TrendingUp size={18} style={{ marginRight: 8 }} /> Matriz de
              Eficiência
            </h3>
          </div>
                    ...
                    return (
                      <div className="custom-tooltip">
                        ...
                        <p className="tooltip-item revenue">
                          Rec: R$ {d.x.toLocaleString("pt-BR")}
                        </p>
                        <p className="tooltip-item profit">
                          Margem: {d.y.toFixed(2)}%
                        </p>
                      </div>
                    );
                  }
        ...
        >
          <div className="flex-header">
            <h3 className="chart-title">
              <PieIcon size={18} style={{ marginRight: 8 }} /> Concentração de
              Risco (Pareto)
            </h3>
          </div>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Tooltip
                ...
                formatter={
                  ((value: any, name: any) => {
                    if (name === "accumulatedPct")
                      return [Number(value).toFixed(2) + "%", "Acumulado"];
                    return [
      ...
      <div
        className="glass-panel chart-container"
        style={{ height: "400px", marginBottom: "32px" }}
      >
        <div className="flex-header">
          <h3 className="chart-title">
            <Layers size={18} style={{ marginRight: 8 }} /> Perfil de Despesas
            (Quem tem o modelo mais pesado?)
          </h3>
        </div>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart
          ...
          >
            ...
            <Tooltip
              ...
              formatter={
                ((value: any) => Number(value).toFixed(2) + "%") as any
              }
            />
            <Legend
              iconSize={10}
      ...
      <div className="glass-panel table-container">
        ...
        >
          <AlertCircle size={14} />
          <span>
            As cores indicam o desvio em relação à <b>Média do Grupo</b> (linha
            azul no topo). Células vermelhas indicam gastos &gt; 20% acima da
            média.
          </span>
        </div>
            ...
            <thead>
              <tr>
                ...
                {categories.map((cat) => (
                  ...
                  >
                    {cat.replace("DESPESAS ", "").replace("COM ", "")}
                  </th>
                ))}
              </tr>
            </thead>
...
const OperationalDiagnosis = ({ data }: { data: FinancialData }) => {
  ...
  const diagnosisData = useMemo(() => {
    const units = data.units.map((u) => {
      const personnelExp =
        u.expenses.find((e) => e.category.includes("PESSOAL"))?.value || 0;
    ...
    const avgRev = activeUnits.length
      ? activeUnits.reduce((acc, u) => acc + u.revenue, 0) / activeUnits.length
      : 0;
    const avgMargin = activeUnits.length
      ? activeUnits.reduce((acc, u) => acc + u.margin, 0) / activeUnits.length
      : 0;

    // Sort by Efficiency
    const efficiencyRanking = [...units].sort(
      (a, b) => b.personnelRoi - a.personnelRoi,
    );

    // Buckets (Safe comparison)
    const buckets = {
      leaders: units.filter(
        (u) => u.revenue >= avgRev && u.margin >= avgMargin,
      ),
      potentials: units.filter(
        (u) => u.revenue < avgRev && u.margin >= avgMargin,
      ),
  ...
  return (
    <div style={{ marginBottom: "32px" }}>
      <h3 className="chart-title" style={{ marginBottom: "16px" }}>
        Diagnóstico Operacional & Clusters
      </h3>
      <div
        className="grid-2-1"
      ...
      >
          ...
          <div className="flex-header">
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              <h4 style={{ fontSize: "0.95rem", margin: 0 }}>
                Eficiência de Pessoal
              </h4>
            </div>
            <span style={{ fontSize: "0.75rem", color: "#94a3b8" }}>
              Faturamento por R$ 1 de Folha
            </span>
          </div>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              data={diagnosisData.efficiencyRanking}
              layout="vertical"
              ...
              >
                {diagnosisData.efficiencyRanking.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={
                      entry.personnelRoi > 10
                        ? "#10b981"
                        : entry.personnelRoi < 4
                          ? "#ef4444"
                          : "#3b82f6"
                    }
                  />
        ...
        >
          ...
          >
            ...
            >
              Alta Receita & Alta Margem. <br />
              As "estrelas" da rede.
            </p>
            <div className="bucket-list">
              {diagnosisData.buckets.leaders.map((u) => (
                <span key={u.id} className="bucket-tag">
                  {u.name}
                </span>
              ))}
            </div>
          ...
          >
            ...
            >
              Baixa Receita, mas Alta Margem. <br />
              Modelos enxutos para replicar.
            </p>
            <div className="bucket-list">
          ...
          >
            ...
            >
              Alta Receita, mas Baixa Margem. <br />
              Risco de operação inchada.
            </p>
            <div className="bucket-list">
          ...
          >
            ...
            >
              Baixo Faturamento & Baixa Margem. <br />
              Necessitam revisão urgente.
            </p>
            <div className="bucket-list">
