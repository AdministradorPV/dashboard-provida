...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<
    "master" | "unit" | "expenses" | "analytics"
  >("master");
  const periods = financeData as unknown as FinancialData[];
  const [activePeriod, setActivePeriod] = useState<string>(
    periods[periods.length - 1]?.meta?.period || "",
  );
  const data: FinancialData =
    periods.find((p) => p.meta?.period === activePeriod) || periods[0];
